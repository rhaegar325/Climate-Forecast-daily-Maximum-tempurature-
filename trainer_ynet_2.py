...
import time
from util.PrepareData import ACCESS_AGCD_dem
...
def evaluation(net,val_dataloders,loss,criterion,args):
    ...
    with torch.no_grad():
        for batch, (pr,dem,hr,_,_) in enumerate(val_dataloders):
            pr,dem,hr = prepare([pr,dem, hr],device,args)
            sr = net(pr,dem)
...
def train_main():
    ...
    # hyper-parameters
    parser.add_argument('--train_name', type=str, default="Ynet_tmax_dem_2_lead1", help='training name')
    ...
    parser.add_argument('--end_date', type=bool, default=datetime(2001,12,24),help='')
    ...
    parser.add_argument('--file_DEM_dir', type=str, default="/scratch/iu60/yz9299/DEM/",help='dataset directory')
...
#         args.file_BARRA_dir="C:/Users/JIA059/barra/"
    ...
    data_set=ACCESS_AGCD_dem(args.start_date,args.end_date,lr_transform=lr_transforms,hr_transform=hr_transforms,args=args)
    if args.test:
        print(1)
    else:
    ...
    #     optimizer_my = optim.SGD(net.parameters(), lr=args.lr, momentum=0.9)
        ...
        #            print("shape of pr",np.shape(pr))
                    ...
                    resize3=transforms.Resize([691,886])
                    sr=resize3(sr)
        #            print("shape of sr & hr",np.shape(sr),np.shape(hr),type(sr))
        #            transform_sr=transforms.Compose([transforms.Resize((691,886)),
        #                                            transforms.ToTensor()])
        #            sr=transform_sr(Image.fromarray(sr.numpy()))

        #            sr=torch.reshape(sr,(691,886))
