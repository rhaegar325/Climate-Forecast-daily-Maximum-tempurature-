...
import time
from util.PrepareData import ACCESS_BARRA_vdsr_pr_dem
from model import vdsr, YNet30_test_dem
import tqdm
...
# hyper-parameters
parser.add_argument('--train_name', type=str, default="Ynet_tmax_crps", help='training name')
...
parser.add_argument('--test_start_time', type=type(datetime(2006,1,1)), default=datetime(2006,1,1),help='a?')
parser.add_argument('--test_end_time', type=type(datetime(2006,12,31)), default=datetime(2006,12,31),help='')

parser.add_argument('--dem', action='store_true',default=True,help='add-on dem?') 
...
parser.add_argument('--file_ACCESS_dir', type=str, default="/scratch/iu60/yz9299/grid_05/tasmax/",help='dataset directory')
parser.add_argument('--file_AGCD_dir', type=str, default="/scratch/iu60/yz9299/grid_05/high_agcd/",help='dataset directory')
...
def main():
    ...
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    net=YNet30_test_dem(num_layers=15,input_channels=1,output_channels=1,scale=8,use_climatology=True)
    # net=torch.load("./save/vdsr_pr/best_test.pth")
    # net=torch.load("../data/model/vdsr_pr/best_test.pth")
    #net=torch.load("./save/Ynet_tmax_re_1/best_train_33.pth",map_location=torch.device('cuda:0'))
    checkpoint=torch.load("./save/Ynet_tmax_dem_2_lead1/best_train_275.pth")
    checkpoint=checkpoint["model"]
    net.load_state_dict(checkpoint.module.state_dict(),strict=True)
    ...
    for lead in range(1,31):
        ...
        #             optimizer.zero_grad()
            with torch.set_grad_enabled(False):
                ...
                sr = net(pr,dem)
                #print('shape of sr', sr.shape)
                resize3=transforms.Resize([691,886])
                sr=resize3(sr)
                sr_np=sr.cpu().numpy()*51
                hr_np=hr.cpu().numpy()*51
                print(sr_np.shape,hr_np.shape)
                # print(sr_np.shape,hr_np.shape)
    ...
    #                 print(data_time,idx)
        ...
        np.save("./save/crps_dem2_2006/"+str(lead)+'_50station',crps_score_vsdr)
